---
title: "Atelier 7: Modèles linéaires (generalisés) à effets mixtes"
...
* Est-ce qu'on s'attend à ce que la position trophique augmente avec la longueur corporelle exactement de la même façon?
...
Dans la littérature des MLMs, vous rencontrerez souvent ces termes. Il existe plusieurs façons de les décrire et nous vous présenterons ici celles que nous trouvons les plus faciles à appliquer.
...
* Le CCI permet de savoir le % de variation dans la variable réponse qui est expliqué par l'effet aléatoire
...
Pour illustrer l'hétérogénéité de la variance, nous allons créer des boîtes à moustaches (boxplots) de la variable réponse par rapport aux différents facteurs environnementaux
...
Pour lancer un GLMM dans `R`, nous faisons appel à la fonction `glmer()` de la librarie lme4.
...
---
# GLMM Poisson - Vérification de la surdispersion

On vérifie la surdispersion en utilisant la fonction `overdisp_fun()` (Bolker *et al*. 2011) qui divise la déviance des résidus (de Pearson) par les degrés de liberté des résidus et teste si le **rapport est plus grand que 1**.
...
Cela est réalisé en ajoutant un **effet aléatoire de niveau d'observation** dans le modèle (voir Harrison (2014)  https://doi.org/10.7717/peerj.616).
...
**Représentons graphiquement les paramètres du modèle**: peut être obtenu en utilisant la fonction `coefplot2()`de la librarie `coefplot2` :

--

.alert[![:faic](warning) Cette librarie n'est pas sur le CRAN! On utilise la librarie remotes pour l'installer depuis GitHub]
...
.alert[Note]: Barres d'erreur visibles seulement pour les effets fixes parce que `glmer()` ne nous donne pas d'information sur l'incertitude des effets aléatoires.
