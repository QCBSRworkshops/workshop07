...
* Ce jeu de données est parfaitement equilibré, mais les **modèles mixtes peuvent analyser les plans experimentaux non equilibrés**, comme c'est souvent le cas en écologie!
...
Il n'est pas recommandé d'inclure des variables explicatives colinéaires dans un même modèle, car leur effet sur la variable réponse sera confondu

Dans cet exemple, il n’y a pas de risque de colinéarité avec seulement une variable continue. Si vous aviez une autre variable continue (Var2) et vous pourriez vérifier la colinéarité à l'aide du code suivant :
...
.comment[Note: Si nous avions différents effets fixes entre les modèles, nous aurions dû indiquer `REML=FALSE`. Cependant, vous devez rapporter les estimations des paramètres du "meilleur" modèle en utilisant `REML=TRUE`]
...
1. Vérifier l'homogénéité des résidus
...
L'étendue homogène des résidus ![:faic](arrow-right) la supposition est respectée!
...
---
## Interpréter et visualiser le modèle

* Les différentes intercepts et pentes générées par le modèle peuvent être illustrées dans des figures pour mieux visualiser les résultats d'un modèle mixte
...
fig <- theme_bw() + 
  ...
  theme(panel.border = element_rect(col = "black", fill=NA))
