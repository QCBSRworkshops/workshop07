...
list.of.packages <- c("ggplot2", "lme4", "AICcmodavg", 'MASS', 'vcdExtra', 'bbmle', 'DescTools')
...
* **REML** to compare **nested random effect models**

* and **ML** to compare **nested fixed effect models**
...
Note: If we had different fixed effects between the models, we would have to specify `REML = FALSE` to compare with likelihood methods like AIC. Here, you must report the estimates of the "best" model parameters using `REML = TRUE`.
...
# order the table from low to high values of AICc
AIC.table <- AIC.table[order(AIC.table$AICc), ]

# add the difference between AICc values and the smallest one (delta AICc)
...
Why using `(1 | Foret / Site)` rather than `(1 | Foret) + (1 | Site)`? See [the answer in ![:faic](stack-exchange)](https://stats.stackexchange.com/questions/228800/crossed-vs-nested-random-effects-how-do-they-differ-and-how-are-they-specified)!
