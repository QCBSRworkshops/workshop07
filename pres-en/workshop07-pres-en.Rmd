...
**5.** Validade, interprete and visualized mixed models with `R`
...
To answer this question, we use a dataset where body lenght was measured in 3 different fish species with 10 individuals sampled per species across 6 different lakes.
...
Note: _MFF: Not sure that mixed models help when the sample size is small. Also, I think that implicitly saying that the data has a hierarchical structure makes more sense_
...
.large[**How could we analyze this data?**]
...
For **our question**, we only want to know if there is a **general effect of length on the trophic position**.
<br>
<br>
...
### Fixed effect : deterministic processes
...
### Random effect : stochastic processes
...
This avoids to estimate an intercept and a slope for each species (+2 parameters per specie) = **saves degrees of freedom** (less parameter estimation is needed)
